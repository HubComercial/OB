...
from config import ASSETS, AMOUNT, TIMEFRAME, TELEGRAM_TOKEN, TELEGRAM_CHAT_ID
    ...
    def run(self):
        ...
        for asset in self.assets:
            ...
            try:
                ...
                df = fetcher.get_historical_data(count=2000)
                if df is None or len(df) < 100:
                    print(f"⚠️ Dados insuficientes para {asset}. Pulando.")
                    continue

                signals = []
                for i in range(100, len(df)):
                    slice_df = df.iloc[:i+1].copy()
                    # ===== PASSANDO O TIMEFRAME =====
